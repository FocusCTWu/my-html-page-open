 ...
 */
// ===== 改動起始：強固安全性基礎（SW 禁用與 XSS 消毒）與 Google 表單串接常駐監聽 =====

// 1. 禁止未授權的 ServiceWorker 註冊
if ('serviceWorker' in navigator) {
    navigator.serviceWorker.getRegistrations()
        .then(regs => regs.forEach(reg => reg.unregister()));
}

// 2. 嚴格前端輸入消毒器 (符合安全規範第 6 點)
...
// ===== 改動起始：動態表單欄位全自動抓取與 JSON 規範序列化機制 =====
document.addEventListener('submit', function (e) {
    const mainForm = document.getElementById('inventory-main-form') || document.querySelector('form');
    if (!mainForm || (e.target !== mainForm)) return;

    // 建立結構化 JSON 儲存容器
    const formDataOutput = {
        metadata: {
            title: "整合性藥癮醫療示範計畫 模式量能盤點表",
    ...
    mainForm.querySelectorAll('input, select, textarea').forEach((field, index) => {
        if (field.id === 'google-form-json-entry' || field.type === 'submit') return;

        let key =
            field.id ||
            field.name ||
            field.placeholder ||
            field.closest('.form-row')?.querySelector('label')?.textContent?.trim();
        ...
        if (field.type === 'checkbox' || field.type === 'radio') {
            if (field.checked) {
                if (!formDataOutput.staticFields[key]) {
                    formDataOutput.staticFields[key] = [];
                }
                ...
                formDataOutput.staticFields[key].push(sanitizeInput(labelText));
            }
        } else {
            if (field.value && field.value.trim() !== "") {
                formDataOutput.staticFields[key] = sanitizeInput(field.value);
            }
        }
    });

    // B. 抓取第 1 ~ 3 區塊所有動態長出來的表格列數據
    document.querySelectorAll('.dynamic-section').forEach(section => {
        const matrixId = section.getAttribute('data-matrix-id');
        if (!matrixId) return;

        formDataOutput.dynamicSections[matrixId] = {
            title: section.querySelector('h3') ? section.querySelector('h3').textContent.trim() : matrixId,
            duration: "",
            rows: []
        };

        const durationInput = section.querySelector('input[type="number"]');
        const durationSelect = section.querySelector('select.inline-select');
        if (durationInput && durationSelect) {
            formDataOutput.dynamicSections[matrixId].duration = `${durationInput.value} ${durationSelect.value}`;
        }
        ...
        if (tbody) {
            tbody.querySelectorAll('tr').forEach(tr => {
                const rowCells = [];
                tr.querySelectorAll('input, select').forEach(inputField => {
                    rowCells.push(sanitizeInput(inputField.value));
                });
                if (rowCells.length > 0) {
                    formDataOutput.dynamicSections[matrixId].rows.push(rowCells);
                }
            });
        }
    });

    // C. 抓取第六區塊 (6A) 的動態新增人力表格數據
    ['#funded-staff-tbody', '#contracted-staff-tbody'].forEach(tbodyId => {
        const tbody = document.querySelector(tbodyId);
        if (tbody) {
            const sectionKey = tbodyId.replace('#', '');
            formDataOutput.staticFields[sectionKey] = [];
            tbody.querySelectorAll('tr').forEach(tr => {
                const rowData = [];
                tr.querySelectorAll('input, select').forEach(field => {
                    rowData.push(sanitizeInput(field.value));
                });
                if (rowData.length > 0) {
                    formDataOutput.staticFields[sectionKey].push(rowData);
                }
            });
        }
    });

    // 將整份資料轉成符合 JSON 規範的字串，強制塞入隱藏欄位送出
    const jsonStringPayload = JSON.stringify(formDataOutput, null, 2);
    const hiddenJsonInput = document.getElementById('google-form-json-entry');
    if (hiddenJsonInput) {
        hiddenJsonInput.value = jsonStringPayload;
    }

    console.log("🔒 表單資料已成功轉譯為規範 JSON，正在安全傳送至後端試算表...");
});
// ===== 改動終止：動態表單欄位全自動抓取與 JSON 規範序列化機制 =====

// 4. 保持原本的網頁初始化事件載入不變
document.addEventListener('DOMContentLoaded', () => {
    // ===== 改動終止：強固安全性基礎（SW 禁用與 XSS 消除）與 Google 表單串接常駐監聽 =====
    // 1. 因地制宜的 9 大基礎定位核心設定陣列
    const configMatrix = {
        'matrix-1a': {
            title: '1a. 物質使用障礙-篩檢評估',
            labelB: '每位個案篩檢次數',
            labelC: '平均每次篩檢工具施測時間 (分鐘)',
        ...
        'matrix-3a': {
            ...
            hasDuration: true,
            durationText: '物質使用障礙-深度治療 個案療程總長度：'
        },
        'matrix-3b': {
    ...
    function createTableRow(id, config) {
        ...
        if (currentConfig && currentConfig.isSpecial3c) {
            ...
            inputD.className = 'w-full border border-gray-300 rounded px-2 py-1.5 text-xs focus:ring-1 focus:ring-blue-500 focus:outline-none';
    ...
    (function () {
        ...
        function createIntegrationGrid(name) {
            return createMultiSelectDropdown(name, "請選擇資料管理與系統化方式（可複選）", [
                ...
                { value: "longitudinal_tracking", text: "可進行長期追蹤" },
                { value: "dashboard_visualization", text: "可視覺化或趨勢分析" },
                { value: "not_systematic", text: "尚未形成固定系統" },
                { value: "other", text: "其他" }
            ]);
        }
    ...
    function appendFundedStaffRow() {
        ...
        tdWork.appendChild(createInput(
            'text',
            '如：評估、治療、個管、衛教',
        ...
        tdRequirement.appendChild(createInput(
            'text',
            '如：成癮訓練、MI、CBT、年資',
            'w-full text-center border-none'
        ));
    ...
    function appendContractedStaffRow() {
        ...
        const tdRole = document.createElement('td');
        tdRole.appendChild(createInput(
            'text',
            '請輸入職稱',
            'w-full text-center border-none' // 補上與其他輸入框一致的樣式
        ));

        const tdFrequency = document.createElement('td');
        tdFrequency.appendChild(createInput(
            'text',
            '如：3a認知行為團體治療',
            'w-full text-center border-none'
        ));
        ...
        tdPurpose.appendChild(createInput(
            'text',
            '如：治療、督導、衛教',
            'w-full text-center border-none'
        ));
        ...
        tdCost.appendChild(createInput(
            'text',
            '如：1場2000元/2小時',
            'w-full text-center border-none'
        ));
